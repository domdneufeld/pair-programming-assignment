...
class Brick {
  ...
  removeBrick() {
    let ballXPosition = myBall.x + myBall.radius;
    let ballYPosition = myBall.y + myBall.radius;
    ...
    for (let x = 0; x < this.cols; x++) {
      for (let y = 0; y < this.rows; y++) {
        ...
        if (setOfBricks[x][y] === 1) {
          if (ballXPosition > this.xPosition + aBrick.width && ballYPosition > this.yPosition + aBrick.height) {
            setOfBricks[x][y] = 0;
            this.yDirection = -this.yDirection;
            this.xDirection = -this.xDirection;
            console.log("hit");
